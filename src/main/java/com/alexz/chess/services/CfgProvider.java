...
public class CfgProvider extends ServiceBase {
  ...
  public void init() {
    ...
    try {
      ...
      for (final Object key : props.keySet()) {
        ...
        } else if (sKey.startsWith("color")) {
          value = this.parseColor((String) value);
          if (value == null) {
            _logger.error("Invalid font format [" + sKey + "]");
            continue;
          }
  ...
  private Color parseColor(final String val) {
    Color color = null;
    if (StringUtils.isNotBlank(val)) {
      final String[] opts = val.split(",");
      if (opts.length == 3) {

        if (NumberUtils.isParsable(opts[0])
            && NumberUtils.isParsable(opts[1])
            && NumberUtils.isParsable(opts[2])) {
          final int r = Integer.parseInt(opts[0]);
          final int g = Integer.parseInt(opts[1]);
          final int b = Integer.parseInt(opts[2]);
